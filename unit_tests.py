...
import unittest
from app import db
from seduce_api.restplus import SensorNotFoundException, SensorNotValidException, PositionNotFoundException, \
...
class TestSeduceApi(unittest.TestCase):
    ...
    def test_update_sensor(self):
        ...
        with self.app.app_context():
            ...
            # Duplicate mac
            with self.assertRaises(SensorNotValidException):
                ser.update_sensor(old_sensor.id,
                                  dict(name="new name", mac="other mac", type="type", model="model", state=0))
            # Duplicate name
            with self.assertRaises(SensorNotValidException):
                ser.update_sensor(old_sensor.id,
                                  dict(name="other name", mac="new mac", type="type", model="model", state=0))
            # OK
            sensor = ser.update_sensor(old_sensor.id,
                                       dict(name="new name", mac="new mac", type="type", model="model", state=0))
            # OK
            sensor = ser.update_sensor(old_sensor.id,
                                       dict(name="another new name", mac="new mac", type="type", model="model",
                                            state=0))
    ...
    def test_get_sensor_history(self):
        ...
        with self.app.app_context():
            ...
            self.assertEqual({"room": position.room, "bus": position.bus, "index": position.index},
                             history[0]["position"])
            self.assertIsNotNone(history[0]["end_of_service"])
            self.assertEqual({"room": position2.room, "bus": position2.bus, "index": position2.index},
                             history[1]["position"])
