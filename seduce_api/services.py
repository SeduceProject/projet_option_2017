...
from seduce_api.restplus import api, SensorNotFoundException, SensorNotValidException, PositionNotFoundException, \
    PositionNotValidException, AssignmentNotFoundException, AssignmentNotValidException, EventNotFoundException
...
def create_event(data):
    ...
    try:
        ...
        raise SensorNotFoundException(
            'No event can be associated with a sensor with id ' + str(id) + ' because it does not exist.')
...
def add_assignment(room, bus, index, data):
    ...
    if get_assignments_aux(room, bus, index).count() > 0:
        raise AssignmentNotValidException(
            'There is already a sensor at position [' + room + ', ' + str(bus) + ', ' + str(index) + '].')
...
def get_assigned_sensor(room, bus, index):
    ...
    if assignments.count() > 0:
        ...
        raise AssignmentNotFoundException(
            'There is no sensor at position [' + room + ', ' + str(bus) + ', ' + str(index) + '].')
...
def get_position_history(room, bus, index):
    return marshalling_aux_position(
        History.query.filter(History.id_position == get_position_by_values(room, bus, index).id))
...
def marshalling_aux_sensor(full_history):
    ...
    for history in full_history:
        ...
        result.append({"start_of_service": history.start_of_service, "end_of_service": history.end_of_service,
                       "position": api.marshal(position, ser_position)})
...
def marshalling_aux_position(full_history):
    ...
    for history in full_history:
        ...
        result.append({"start_of_service": history.start_of_service, "end_of_service": history.end_of_service,
                       "sensor": api.marshal(sensor, ser_sensor)})
